...
# Calibration is only identifiable when predictions vary and both outcome classes are
# present. Perfectly separating or constant predictions drive the logistic slope to
# +/-infinity, so the fit is bounded and reported as NaN when it pins to a bound.
_CAL_SLOPE_BOUNDS = (-10.0, 10.0)
_CAL_INTERCEPT_BOUNDS = (-10.0, 10.0)
_CAL_MIN_LP_STD = 1e-9


def calibration_slope(y_true: np.ndarray, y_pred: np.ndarray) -> tuple[float, float]:
    """(slope, intercept) of the calibration logistic; (NaN, NaN) if not identifiable."""
    ...
    if not np.all(np.isfinite(y_pred)) or not np.all(np.isfinite(y_true)):
        return math.nan, math.nan
    if len(np.unique(y_true)) < 2:
        return math.nan, math.nan  # single-class outcome: nothing to calibrate against

    p = np.clip(y_pred, 1e-6, 1 - 1e-6)
    lp = logit(p)
    if float(np.std(lp)) < _CAL_MIN_LP_STD:
        return math.nan, math.nan  # constant predictor
    ...
    # NOTE: the optimiser is deliberately unchanged (Nelder-Mead) so that well-behaved
    # cohorts return bit-identical values to previous releases; hardening is done by the
    # guards above and the admissibility check below, not by swapping the algorithm.
    ...
    if not math.isfinite(slope) or not math.isfinite(intercept):
        return math.nan, math.nan
    if not (_CAL_SLOPE_BOUNDS[0] <= slope <= _CAL_SLOPE_BOUNDS[1]):
        # Separation / flat likelihood: the slope has run to an implausible magnitude.
        return math.nan, math.nan
