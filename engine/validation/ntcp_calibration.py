...
class FittedNTCPParams:
    ...
    n_events: int
    converged: bool
    # False when the optimiser pinned to a parameter bound or failed: the returned TD50/m are
    # then NOT a usable dose-response estimate, only the starting point. Callers must not
    # report a non-identifiable fit as a calibrated model.
...
def _neg_log_likelihood_lkb(
    ...
    if td50 <= 0 or m <= 0 or n <= 0 or n > 1.5:
        return 1e10
    nll = 0.0
    for dvh, yi in zip(dvh_list, outcomes, strict=False):
...
def _bootstrap_ci_lkb(
    ...
    if successes < 20:
        logger.warning(
            "Only %d/%d bootstrap resamples converged for CI. " "CI may be unreliable.",
            successes,
            n_bootstrap,
        )
...
def fit_lkb_parameters(
    ...
    # Identifiability: a fit pinned to a bound (or a failed fit) is not a dose-response
    # estimate. Report it rather than returning an extreme/starting value as if it were one.
    fit_note = ""
    identifiable = bool(res.success)
    if not res.success:
        fit_note = "optimiser did not converge; TD50/m are the starting values, not a fit"
    else:
        at_td50_bound = abs(td50_fit - bounds[0][0]) < 1e-6 or abs(td50_fit - bounds[0][1]) < 1e-6
        at_m_bound = abs(m_fit - bounds[1][0]) < 1e-9 or abs(m_fit - bounds[1][1]) < 1e-9
        if at_td50_bound or at_m_bound:
            identifiable = False
            fit_note = (
                "not identifiable (flat/degenerate likelihood): fit pinned to a parameter "
                "bound; treat TD50/m as unusable"
            )
    if not identifiable:
        logger.warning("LKB calibration for %s/%s: %s", organ, site, fit_note)
