...
def _require_lgbm() -> None:
    if not _LGBM_AVAILABLE:
        raise ImportError(
            "lightgbm>=3.3 required for LightGBM TCP model. "
...
def fit_lgbm_tcp(
    ...
    """LightGBM with nested group CV for TCP binary outcome prediction."""
    _require_lgbm()
    X_arr = np.asarray(X, dtype=float)
    y_arr = np.asarray(y, dtype=int)
    n, n_f = X_arr.shape
    fname = list(feature_names) if feature_names else [f"feat_{i}" for i in range(n_f)]
    pg = param_grid or LGBM_PARAM_GRID
    warns: list[str] = []
    ...
    final_pipe = _make_lgbm_pipeline()
