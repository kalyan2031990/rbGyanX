...
def calculate_ntcp_rs_poisson(
    ...
    vol_col = (
        ...
        else "volume_frac" if "volume_frac" in dvh.columns else "volume"
    )
    ...
    try:
        ...
        # Källman relative seriality:
        #     NTCP = [ 1 - PROD_i (1 - P_i^s)^{v_i} ]^{1/s}
        # Each factor is the sub-volume's "no complication" contribution; the product runs
        # over bins and only then is complemented. (Complementing inside the product makes
        # every low-dose bin contribute ~0, collapsing NTCP to 1.0 for every patient.)
        survival = np.power(1.0 - np.power(p_voxel, s), v_rel)
        survival = np.clip(survival, 0.0, 1.0)
        product = float(np.prod(survival))
