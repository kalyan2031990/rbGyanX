...
def therapeutic_window(
    ...
    mask = np.isfinite(d) & np.isfinite(tcp) & np.isfinite(ntcp) & (tcp >= tau_t) & (ntcp <= tau_n)
...
def compute_utcp_p_plus(
    ...
    if not math.isfinite(utcp):
        return float("nan")
    survival = 1.0
    values = untcp_by_oar.values() if isinstance(untcp_by_oar, Mapping) else untcp_by_oar
    for p in values:
        if math.isfinite(p):
