...
class TCPCalculator:
    ...
    def compute_all(
        ...
        try:
            poisson = self._poisson.compute_tcp_dvh(dvh_df, n_fractions, site_params, target)
        ...
        model_tcps: list[float] = [
            float(result["TCP_Poisson"]),
            float(result["TCP_ZM"]),
            float(result["TCP_gEUD"]),
            float(result["TCP_Logistic"]),
        ]
