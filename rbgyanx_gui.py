...
"""
    ...
    def build_canonical_dvh(self, dose, volume, dvh_type):
        """Build canonical cumulative and differential DVH (pure).

        Thin view over :func:`rbgyanx.services.build_canonical_dvh`.
        """
        return _svc_build_canonical_dvh(dose, volume, dvh_type)
    def normalize_dvh(self, dose, volume, dvh_type):
        """Normalise a DVH to cumulative percentage form.

        Thin view over :func:`rbgyanx.services.normalize_dvh`.
        """
        return _svc_normalize_dvh(dose, volume, dvh_type)
    def validate_dvh_integrity(self, dvh_df, dvh_type, structure_name, patient_id):
        """Validate DVH integrity (read-only, non-blocking).

        Thin view over :func:`rbgyanx.services.validate_dvh_integrity` — the single
        implementation shared by this app, the Qt app and headless runs.
        """
        return _svc_validate_dvh_integrity(dvh_df, dvh_type, structure_name, patient_id)
